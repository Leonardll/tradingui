...
export async function fetchMyTrade (symbol: string | string[] | ParsedQs | ParsedQs[], orderId?: number): Promise<any> {
    ...
    try {
        const response  = await axios.get(`${testApiUrl}/myTrades?${queryString}&signature=${signature}`, {
            headers: {
...
export async function fetchCurrentOrderCount(): Promise<any> {
    ...
    try {
        const response = await axios.get(`${testApiUrl}/rateLimit/order?${queryString}&signature=${signature}`, {
            headers: {
                "X-MBX-APIKEY": testApiKey,
                "CACHE-CONTROL": "no-cache",
            },
...
export async function fetchPreventedMatches(params: PreventedMatchesParams): Promise<any> {
    ...
    if (params.fromPreventedMatchId) queryString += `&fromPreventedMatchId=${params.fromPreventedMatchId}`;
    ...
    try {
        const response = await axios.get(`${testApiUrl}/v3/preventedMatches?${queryString}&signature=${signature}`, {
            headers: {
                "X-MBX-APIKEY": testApiKey,
                "CACHE-CONTROL": "no-cache",
            },
