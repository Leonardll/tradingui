...
export const getSymbolFilters = async ( exchangeName: string, tradingPair: string): Promise<Symbol | null> => {
    const exchange: IExchange | null = await ExchangeModel.findOne({exchangeName: exchangeName });
    if (!exchange || !exchange.exchangeInfo) {
      return null;
    }
  
    const symbolInfo: Symbol | undefined = exchange.exchangeInfo.symbols.find((symbol) => symbol.symbol === tradingPair);
